...
class MLfuntions:
    ...
    def backpropagation(self,node1,node2,node3,outputnode):#Applies the backpropagation function
        ...
        for i in range(numexamples): #For each input set
            ...
            #Inverses sigmoid function -> applies diffierential -> applies hadamard product with error -> multiplies hidden layer by result -> multiplies by learning rate
            adjustmentsbias4+=self.learningrate*error4*self.sigmoid_diff(self.inv_sigmoid(outputnode[i]))
            #Inverses sigmoid function -> applies diffierential -> applies hadamard product with error -> multiplies by learning rate
