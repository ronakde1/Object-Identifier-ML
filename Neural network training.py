...
class MLfuntions:
    def __init__(self,index,w01,w12,w23,w34,b1,b2,b3,b4):
        ...
        self.object=np.array(objects,np.float64)
        self.learningrate=0.5
        self.weights01=w01.astype(float);self.weights12=w12.astype(float);self.weights23=w23.astype(float);self.weights34=w34.astype(float)
        self.bias1=b1.astype(float);self.bias2=b2.astype(float);self.bias3=b3.astype(float);self.bias4=b4.astype(float)
    ...
    def backpropagation(self,node1,node2,node3,outputnode):#Applies the backpropagation function
        ...
        for i in range(numexamples): #For each input set
            ...
            error3=np.matmul(self.weights34,error4) #Propagating the error backwards using tranpose of weight matrix 
            adjustmentweights23+=self.learningrate*np.outer(error3*self.sigmoid_diff(self.inv_sigmoid(node3[i])),node2[i])
            #Applies weight adjustment algorithm to hidden layer (line 224)
            adjustmentsbias3+=self.learningrate*error3*self.sigmoid_diff(self.inv_sigmoid(node3[i]))
            #Applies bias adjustment algorithm to hidden layer (line 226)

            #Repeats previous steps for between 2nd layer and 3rd layer
            error2=np.matmul(self.weights23,error3)
            ...
            error1=np.matmul(self.weights12,error2)
        ...
        self.weights34+=adjustmentweights34.T/numexamples
        self.weights23+=adjustmentweights23.T/numexamples
        self.weights12+=adjustmentweights12.T/numexamples
        self.weights01+=adjustmentweights01.T/numexamples
        self.bias1+=adjustmentsbias1.T/numexamples
        self.bias2+=adjustmentsbias2.T/numexamples
        self.bias3+=adjustmentsbias3.T/numexamples
        self.bias4+=adjustmentsbias4.T/numexamples
